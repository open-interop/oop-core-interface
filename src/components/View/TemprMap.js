...
import { Page, DataProvider } from "../Universal";
...
const TemprMap = props => {
    ...
    const formatNode = temprObj => {
        return `${temprObj.id}[shape=plain, fontname=Helvetica,
				label=<
				<table border="${temprObj.id === props.match.params.temprId ? "2" : "1"}" color="${
            temprObj.id === props.match.params.temprId ? "#177692" : "black"
        }" cellborder="0" cellspacing="0" cellpadding="2">
			    	<tr>
			     		<td align="left" colspan="8">
			     			<font point-size="16" color="${
                                temprObj.id === props.match.params.temprId ? "#177692" : "black"
                            }"><u>${temprObj.name}</u></font>
		     			</td>
		     			<td colspan="1" align="right">
			     			<table cellborder="0" cellspacing="1" border="0" cellpadding="0">
			     				...
			     				<tr>
			     					<td></td>
			     					<td></td>
			     					<td cellpadding="1" colspan="1" bgcolor="#177692" tooltip="Edit ${
                                        temprObj.name
                                    }" href="/temprs/${temprObj.id}">
			     	...
			     	<tr>
			     		<td colspan="6">
			     			<table cellborder="0" color="black" cellspacing="2" border="0" cellpadding="5">
			     				<tr>
			     					<td align="text" colspan="2" bgcolor="${
                                        temprObj.queueRequest ? "#28364D" : "#D3D3D3"
                                    }"><font point-size="16" color="white">&#8659;</font></td>
			     					<td align="text" colspan="2" bgcolor="${
                                        temprObj.queueResponse ? "#28364D" : "#D3D3D3"
                                    }"><font point-size="16" color="white">&#8657;</font></td>
			     					<td colspan="1">&nbsp;</td>
        ...
        if (ps && cs) {
            ...
            while (childrenData.length > 0) {
                ...
                if (c.temprId) {
                    ...
                    var newChildren = await OopCore.getTemprs({
                        temprId: c.id,
                    });
                    newChildren = newChildren.data;
                    for (var q = newChildren.length - 1; q >= 0; q--) {
                        if (!nodes[newChildren[q].id] && newChildren[q].temprId === c.id) {
                            childrenData.push(newChildren[q]);
                        }
                    }
                }
            }
